...
PROCESSED_DIR = '/gpfs/scratch/dk5565/phase41_data/processed'
...
def main(num_epochs=100, num_samples=5):
    output_dir = os.path.join(BASE_DIR, 'outputs', f'iTransformer_epochs_{num_epochs}')
    ...
    historical = test_data['historical_ts_numeric'].numpy()  # (N, 75, 9)
    # First 4 channels are the normalized vitals
    hist_vitals = historical[:, :, :NUM_SIGNALS]  # (N, 75, 4)
    ...
    # Get historical masks (channels 4-7)
    hist_masks = historical[:, :, NUM_SIGNALS:NUM_SIGNALS*2]  # (N, 75, 4)
    ...
    for plot_i, idx in enumerate(indices):
        ...
        for s in range(NUM_SIGNALS):
            ...
            # Plot historical (real values solid, imputed dashed)
            ax.plot(hist_hours, hist_vals, '-', color='tab:blue', lw=1.2, alpha=0.8, label='Input (history)')

            # Mark imputed historical values
            imputed_hist = hist_mask == 0
            if imputed_hist.any():
                ax.scatter(hist_hours[imputed_hist], hist_vals[imputed_hist],
                          marker='.', color='tab:blue', alpha=0.3, s=10)
        ...
        axes[0].set_title(f'iTransformer — Full Forecast (Sample {plot_i + 1})\n'
