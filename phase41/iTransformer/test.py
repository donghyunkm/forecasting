#!/usr/bin/env python3
"""
test.py - Evaluation for iTransformer vital sign forecasting (Phase 41).

Adapted from TFT test.py, modified for iTransformer architecture:
...
def get_checkpoint_dir(num_epochs=NUM_EPOCHS):
    return os.path.join(BASE_DIR, 'checkpoints', f'iTransformer_epochs_{num_epochs}')
...
def test_model(device, num_epochs=NUM_EPOCHS):
    ...
    Evaluation logic adapted from TFT-multi metric calculation,
    updated for iTransformer with 4 vitals at 15-minute resolution.
    """
    ...
    # Get normalization params from processed data directory
    norm_json = os.path.join('/gpfs/scratch/dk5565/phase41_data/processed', 'norm_params.json')
    with open(norm_json, 'r') as f:
        norm_data = json.load(f)
    norm_mean = np.array(norm_data['mean'])   # (4,)
    norm_std = np.array(norm_data['std'])     # (4,)

    # Recreate model
    model = iTransformer(
        seq_len=75, pred_len=25, n_vars=4,
        n_output_vars=4, n_quantiles=3,
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            output = model(batch)
            predicted_quantiles = output['predicted_quantiles']  # (B, 25, 12)
            prediction = predicted_quantiles.cpu().numpy()
    ...
    predictions = np.concatenate(all_predictions, axis=0)  # (N, 25, 12)
    targets = np.concatenate(all_targets, axis=0)          # (N, 25, 4) normalized
    masks = np.concatenate(all_masks, axis=0)              # (N, 25, 4)

    # Reshape predictions: (N, 25, 12) → (N, 25, 4, 3)
    ...
    for meas in range(NUM_SIGNALS):
        ...
        # MAE/MAPE per quantile (only on real values)
    ...
    pred_median = predictions_denorm[:, :, :, median_idx]  # (N, 25, 4)
    ...
    metrics = {
        ...
        'forecast_hours': FUTURE_MONTHS * 15 / 60,  # 6.25 hours
        'num_signals': NUM_SIGNALS,
        'signal_names': SIGNAL_NAMES,
        'quantiles': OUTPUT_QUANTILES,
        'input_steps': PAST_MONTHS,
        'input_hours': PAST_MONTHS * 15 / 60,  # 18.75 hours
...
def run_test(num_epochs=NUM_EPOCHS):
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("TEST RESULTS — iTransformer Vital Sign Forecasting (Phase 41)")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Evaluate iTransformer forecaster (Phase 41 — 4 vitals, 15-min)')
