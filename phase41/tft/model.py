...
class InputChannelEmbedding(nn.Module):
    def __init__(self, state_size: int, num_numeric: int, num_categorical: int,
        ...
        if (num_numeric + num_categorical) < 1:
            raise ValueError(
                f"At least a single input variable (either numeric or categorical) should be "
                f"included as part of the input channel. "
                f"num_numeric + num_categorical = {num_numeric} + {num_categorical} = "
                f"{num_numeric + num_categorical} < 1")
...
class TemporalFusionTransformer(nn.Module):
    ...
    def apply_temporal_selection(self, temporal_representation: torch.tensor,
        ...
        temporal_selection_output, temporal_selection_weights = temporal_selection_module(
            flattened_embedding=temporal_flattened_embedding,
    ...
    def apply_sequential_processing(self, selected_historical: torch.tensor,
                                     selected_future: torch.tensor,
                                     c_seq_hidden: torch.tensor,
                                     c_seq_cell: torch.tensor) -> torch.tensor:
