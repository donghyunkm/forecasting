#!/usr/bin/env python3
"""
plot_predictions.py - Generate forecast visualizations for Phase 41.

Adapted from Phase 4 for 4 vitals at 15-minute resolution:
...
def plot_forecast_samples(predictions, targets, masks, output_dir,
    ...
    For each sample:
      ...
      - Historical context (300 steps = 75 hours)
      - Actual future (100 steps = 25 hours)
      - Predicted median + 10th-90th confidence interval
      - Vertical line at hour 75 separating history from forecast
      - X-axis in hours
    """
    ...
    if has_history:
        historical = np.load(hist_path)  # (N, 300, 4)
    else:
        historical = None
    ...
    for sample_i in range(min(num_samples, len(predictions))):
        ...
        for meas in range(NUM_SIGNALS):
            ...
            pred_meas = predictions[idx, :, meas, :]  # (100, 3)
            true_meas = targets[idx, :, meas]          # (100,)
            mask_meas = masks[idx, :, meas]            # (100,)

            # Future time axis (starts at hour 75)
            ...
            # Vertical line at forecast boundary (hour 75)
            history_end_hour = PAST_MONTHS / STEPS_PER_HOUR  # 75.0
        ...
        plt.suptitle(
            f'TFT-multi Forecast — Phase 41 — Sample {sample_i + 1}\n'
            f'(4 vitals, 15-min resolution, {FUTURE_MONTHS} steps = '
            f'{FUTURE_MONTHS / STEPS_PER_HOUR:.0f}h forecast)',
            fontsize=13)
        plt.tight_layout()
...
def plot_scatter_per_vital(predictions, targets, masks, output_dir):
    ...
    plt.suptitle('TFT-multi Phase 41 — Predicted vs Actual (Median)',
                 fontsize=14)
    plt.tight_layout()
...
def plot_error_by_step(predictions, targets, masks, output_dir):
    ...
    ax.set_ylabel('MAE')
    ax.set_title('TFT-multi Phase 41 — MAE by Forecast Step (15-min resolution)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_calibration_summary(predictions, targets, masks, output_dir):
    ...
    ax.set_title('TFT-multi Phase 41 — Prediction Interval Calibration')
    ax.set_ylim(0, 1.0)
    ax.legend()
...
def main(num_epochs=None):
    ...
    predictions = np.load(pred_path)    # (N, 100, 4, 3)
    targets = np.load(tgt_path)          # (N, 100, 4)
    masks = np.load(mask_path)           # (N, 100, 4)
    ...
    print(f"[INFO] Resolution: 15 min, forecast: {FUTURE_MONTHS} steps "
          f"= {FUTURE_MONTHS / STEPS_PER_HOUR:.0f} hours")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Plot TFT-multi results (Phase 41 — 4 vitals, 15-min)')
