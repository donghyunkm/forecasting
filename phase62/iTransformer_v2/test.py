"""Phase 6.2 v2 iTransformer Testing."""
...
def main():
    ...
    output_dir = f"outputs/itransformer_v2_epochs_{args.epochs}"
    ...
    print(f"\n{'='*70}")
    print(f"  iTransformer v2 Test Results (with label history)")
    print(f"{'='*70}")
    ...
    ax.set_title('iTransformer v2 — Confusion Matrix'); plt.tight_layout()
    ...
    ax.set_title('iTransformer v2 — Accuracy by Horizon'); ax.legend(); ax.grid(True, alpha=0.3)
    ...
    ax.set_ylabel('F1'); ax.set_title('iTransformer v2 — Per-Class F1'); ax.legend()
