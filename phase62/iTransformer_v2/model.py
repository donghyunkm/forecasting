"""
iTransformer for Phase 6.2: Cluster Label Forecasting (Classification).
...
class iTransformer(nn.Module):
    """
    iTransformer for cluster label classification (Phase 6.2).
    ...
    def __init__(
        ...
        # Approach: pool variate tokens → project to (pred_len * num_classes)
    ...
    def forward(self, x):
        ...
        pooled = enc_out.reshape(B, -1)  # (B, 8 * d_model)
...
def build_model(device='cuda'):
    """Build iTransformer with default config for Phase 6.2 v2 (with label history)."""
    model = iTransformer(
        seq_len=48,
        pred_len=12,
        n_input_vars=9,       # 7 corr + 1 time + 1 label history
        num_classes=7,
        d_model=256,
        n_heads=4,
        d_ff=512,
...
if __name__ == "__main__":
    ...
    x = torch.randn(4, 48, 9).to(device)
    out = model(x)
