#!/usr/bin/env python3
"""Phase 6.2 v2 TFT Testing: Evaluate cluster classification with label history input."""
...
def main():
    ...
    checkpoint_dir = os.path.join(base_dir, f"checkpoints/tft_v2_epochs_{args.epochs}")
    output_dir = os.path.join(base_dir, f"outputs/tft_v2_epochs_{args.epochs}")
    ...
    print(f"\n{'='*70}")
    print(f"TEST RESULTS — Phase 6.2 v2 TFT (with label history)")
    print(f"{'='*70}")
    ...
    ax.set_title('TFT v2 — Confusion Matrix'); plt.tight_layout()
    ...
    ax.set_title('TFT v2 — Accuracy by Horizon'); ax.legend(); ax.grid(True, alpha=0.3)
    ...
    ax.set_ylabel('F1'); ax.set_title('TFT v2 — Per-Class F1'); ax.legend(); ax.grid(True, alpha=0.2, axis='y')
