#!/usr/bin/env python3
"""
Phase 6.2 v2 TFT Training: Cluster classification with label history input.

Input: 48 steps × 9 features (7 correlations + 1 time + 1 label history)
...
def get_config():
    config = OmegaConf.create({
        ...
        'data_props': {
            'num_historical_numeric': 9,      # 7 corr + 1 time + 1 label history
...
def main():
    ...
    checkpoint_dir = os.path.join(base_dir, f"checkpoints/tft_v2_epochs_{EPOCHS}")
    output_dir = os.path.join(base_dir, f"outputs/tft_v2_epochs_{EPOCHS}")
    ...
    print("\nLoading data (v2 — with label history)...")
