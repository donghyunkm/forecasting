#!/usr/bin/env python3
"""
test.py - Evaluation for iTransformer vital sign forecasting (Phase 4.3).
...
def get_checkpoint_dir(horizon, num_epochs=NUM_EPOCHS):
    return os.path.join(BASE_DIR, 'checkpoints', f'iTransformer_{horizon}_epochs_{num_epochs}')
...
def test_model(device, horizon, num_epochs=NUM_EPOCHS):
    ...
    norm_json = os.path.join(
        '/gpfs/scratch/dk5565/phase43_data/processed',
        f'horizon_{pred_len}', 'norm_params.json')
    ...
    # Recreate model
    model = iTransformer(
        seq_len=75, pred_len=pred_len, n_vars=4,
        n_output_vars=4, n_quantiles=3,
        d_model=256, n_heads=4, d_ff=512,
        n_layers=3, dropout=0.1, use_mask_input=False
    ).to(device)
    model.load_state_dict(checkpoint['model_state_dict'])
    model.eval()

    _, _, test_loader, _ = create_dataloaders(horizon=horizon, batch_size=BATCH_SIZE)
    ...
    # Reshape predictions: (N, pred_len, 12) -> (N, pred_len, 4, 3)
    predictions = predictions.reshape(
        predictions.shape[0], pred_len, NUM_SIGNALS, len(OUTPUT_QUANTILES))
...
def run_test(horizon, num_epochs=NUM_EPOCHS):
    ...
    print("\n" + "=" * 60)
    print(f"TEST RESULTS \u2014 iTransformer Vital Sign Forecasting (Phase 4.3, {horizon})")
    print("=" * 60)
    print(f"  Resolution: 15 minutes")
    print(f"  Input:  {PAST_MONTHS} steps ({PAST_MONTHS * 15 / 60:.1f} hours)")
    print(f"  Output: {pred_len} steps ({pred_len * 15 / 60:.1f} hours) "
          f"x {NUM_SIGNALS} vitals x {len(OUTPUT_QUANTILES)} quantiles")
    ...
    print(f"  ---")
    print(f"  Per-vital (MAE q0.5 / MAPE q0.5 / Calibration / Correlation):")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Evaluate iTransformer forecaster (Phase 4.3)')
