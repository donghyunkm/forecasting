...
Shows the complete picture: 18.75h of historical input followed by forecast
with median prediction and 10th-90th confidence band.

Phase 4.3: Multi-horizon forecasting. Predictions are already denormalized from test.py.
...
def main(horizon='6h', num_epochs=100, num_samples=5):
    pred_len = HORIZON_MAP[horizon]
    output_dir = os.path.join(BASE_DIR, 'outputs', f'iTransformer_{horizon}_epochs_{num_epochs}')
    processed_dir = f'/gpfs/scratch/dk5565/phase43_data/processed/horizon_{pred_len}'

    # Load predictions, targets, masks (ALREADY denormalized from test.py)
    ...
    # Load normalization params (for denormalizing historical)
    ...
    historical = test_data['historical_ts_numeric'].numpy()  # (N, 75, 5) -- 4 vitals + 1 time

    # First 4 channels are the normalized vitals
    hist_vitals = historical[:, :, :4]  # (N, 75, 4)

    # Denormalize historical
    ...
    print(f"[INFO] Horizon: {horizon} ({pred_len} steps = {pred_len / STEPS_PER_HOUR:.1f} hours)")
    ...
    for plot_i, idx in enumerate(indices):
        ...
        for s in range(NUM_SIGNALS):
            ...
            # Future
            future_hours = steps_to_hours(np.arange(PAST_MONTHS, PAST_MONTHS + pred_len))
            true_vals = targets_denorm[idx, :, s]
            pred_vals = preds_denorm[idx, :, s, :]  # (pred_len, 3)
            fut_mask = masks[idx, :, s]

            # Plot historical
            ax.plot(hist_hours, hist_vals, '-', color='tab:blue', lw=1.2, alpha=0.8,
                    label='Input (history)')
            ...
            # Mark imputed future values
            imputed_fut = fut_mask == 0
            if imputed_fut.any():
                ax.scatter(future_hours[imputed_fut], true_vals[imputed_fut],
                          marker='x', color='red', s=25, zorder=5, label='Imputed')
        ...
        axes[0].set_title(
            f'iTransformer Phase 4.3 ({horizon}) \u2014 Full Forecast (Sample {plot_i + 1})\n'
            f'Input: 0\u2013{PAST_MONTHS/STEPS_PER_HOUR:.1f}h | '
            f'Forecast: {PAST_MONTHS/STEPS_PER_HOUR:.1f}\u2013'
            f'{(PAST_MONTHS+pred_len)/STEPS_PER_HOUR:.1f}h',
...
if __name__ == '__main__':
    ...
    main(horizon=args.horizon, num_epochs=args.epochs, num_samples=args.num_samples)
