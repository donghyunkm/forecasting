#!/usr/bin/env python3
"""
plot_predictions.py - Generate forecast visualizations for Phase 4.3.

Phase 4.3 trains separate models for 1h/3h/6h forecast horizons.
Accepts --horizon flag to select which model's outputs to plot.

Plots generated:
  1. plot_forecast_sample_{i}.png: Sample forecasts with 4 subplots
...
def plot_forecast_samples(predictions, targets, masks, output_dir,
                          horizon, future_months, num_samples=5):
    """
    ...
    for sample_i in range(min(num_samples, len(predictions))):
        ...
        for meas in range(NUM_SIGNALS):
            ...
            pred_meas = predictions[idx, :, meas, :]  # (FUTURE_MONTHS, 3)
        ...
        plt.suptitle(
            f'TFT Forecast \u2014 {horizon} Forecast (Phase 4.3) \u2014 Sample {sample_i + 1}\n'
...
def plot_scatter_per_vital(predictions, targets, masks, output_dir, horizon):
    ...
    plt.suptitle(f'TFT Phase 4.3 ({horizon}) \u2014 Predicted vs Actual (Median)',
                 fontsize=14)
    plt.tight_layout()
...
def plot_error_by_step(predictions, targets, masks, output_dir, horizon, future_months):
    ...
    ax.set_ylabel('MAE')
    ax.set_title(f'TFT Phase 4.3 ({horizon}) \u2014 MAE by Forecast Step (15-min resolution)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_calibration_summary(predictions, targets, masks, output_dir, horizon):
    ...
    ax.set_title(f'TFT Phase 4.3 ({horizon}) \u2014 Prediction Interval Calibration')
    ax.set_ylim(0, 1.0)
    ax.legend()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Plot TFT results (Phase 4.3 \u2014 multi-horizon)')
