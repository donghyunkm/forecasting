#!/usr/bin/env python3
"""
run_pipeline.py - Orchestrator for diffusion waveform forecasting (Phase 3).

Runs the full pipeline for a given target signal:
    1. Download data (if not already present)
    2. Train diffusion model
    3. Test model (reverse diffusion sampling)
    4. Plot predictions

Usage:
    python run_pipeline.py --target II
    python run_pipeline.py --target PLETH --epochs 100
"""
...
def run_testing(target_signal, num_epochs):
    """Test model via reverse diffusion."""
    print("\n" + "=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run full forecasting pipeline (Diffusion)')
    ...
    print("=" * 60)
    print(f"Phase 3 — Multivariate Waveform Forecasting Pipeline (Diffusion)")
    print("=" * 60)
