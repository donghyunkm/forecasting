#!/usr/bin/env python3
"""
test.py - Evaluation script for diffusion waveform forecasting (Phase 3).

Loads the best checkpoint, generates forecasts via reverse diffusion,
evaluates on the test set, and saves metrics.

Usage:
    python test.py --target II
    python test.py --target PLETH --epochs 100
"""
...
from model import (ConditionalDenoiser, get_checkpoint_dir, get_output_dir,
                   get_beta_schedule, get_diffusion_params, sample,
                   NUM_EPOCHS, NUM_TIMESTEPS, INPUT_SIZE, OUTPUT_FLAT_SIZE)
...
def test_model(device, target_signal='II', num_epochs=NUM_EPOCHS):
    """
    Load best checkpoint, generate forecasts via reverse diffusion, evaluate.
    ...
    norm_mean = np.array(norm_params['norm_mean'])
    norm_std = np.array(norm_params['norm_std'])

    # Denormalization for all 6 features of target signal
    ...
    print(f"[INFO] Generating forecasts via reverse diffusion (T={NUM_TIMESTEPS})...")

    # Run inference via reverse diffusion
    all_predictions = []
    all_targets = []

    with torch.no_grad():
        for batch_idx, (x_cond, y_target) in enumerate(test_loader):
            x_cond = x_cond.to(device)
            # Generate forecast: sample returns (batch, 150)
            predictions_flat = sample(model, x_cond, diffusion_params, device)
            # Reshape to (batch, 25, 6)
            predictions = predictions_flat.view(-1, OUTPUT_LENGTH, NUM_FEATURES)
            all_predictions.append(predictions.cpu().numpy())
            all_targets.append(y_target.numpy())  # already (batch, 25, 6)
    ...
    # Denormalize — broadcast (6,) over (N, 25, 6)
    ...
    per_step_mae = np.mean(np.abs(predictions_denorm - targets_denorm), axis=(0, 2))
...
def run_test(target_signal='II', num_epochs=NUM_EPOCHS):
    ...
    # Print summary
    print("\n" + "=" * 60)
    print(f"TEST RESULTS — {target_signal} Forecasting (Diffusion)")
    print("=" * 60)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Test Diffusion forecaster')
