"""
iTransformer for Phase 6: Correlation-Only Forecasting.
...
All correlation tokens project to point predictions.

Input: (B, 48, 8) = 7 correlations + 1 time position
...
class iTransformer(nn.Module):
    """
    iTransformer for point-prediction correlation forecasting (Phase 6).

    Input: (B, seq_len=48, n_input_vars=8)
        - indices 0-6: 7 correlation features
        - index 7: time feature
    ...
    def __init__(
        ...
        seq_len: int = 48,
        pred_len: int = 12,
        n_vars: int = 7,           # 7 correlations (excluding time)
        n_input_vars: int = 8,     # 7 correlations + 1 time
    ...
    def forward(self, x):
        """
        Args:
            x: (B, seq_len, n_input_vars) = (B, 48, 8)
        ...
        # Embed: (B, 48, 8) -> (B, 8, d_model)
        enc_in = self.embedding(x)

        # Encode with transformer (attention across 8 variate tokens)
        enc_out = self.encoder(enc_in)  # (B, 8, d_model)
...
def build_model(device='cuda'):
    """Build iTransformer with default config for Phase 6."""
    model = iTransformer(
        seq_len=48,
        pred_len=12,
        n_vars=7,
...
if __name__ == "__main__":
    ...
    # Test forward pass
    x = torch.randn(4, 48, 8).to(device)
    out = model(x)
