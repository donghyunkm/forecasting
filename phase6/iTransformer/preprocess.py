"""
Data loading for Phase 6 iTransformer — Correlation-Only Forecasting.
...
DATA_DIR = "/gpfs/home/dk5565/forecasting/phase6/phase6_data/processed"
...
FEATURE_NAMES = CORRELATION_NAMES + ['time']
...
class PreloadedDataset(Dataset):
    ...
    def __init__(self, data_dict):
        ...
        """
        self.historical = data_dict['historical_ts_numeric'].float()  # (N, 48, 8) — already normalized
...
def create_dataloaders(batch_size=64, num_workers=4, data_dir=None):
    ...
    means = np.array(norm_data['means'])  # (7,)
    stds = np.array(norm_data['stds'])    # (7,)
    norm_params = {
        'corr_mean': torch.tensor(means, dtype=torch.float32),
        'corr_std': torch.tensor(stds, dtype=torch.float32),
        'means': norm_data['means'],
        'stds': norm_data['stds'],
        'feature_names': norm_data['feature_names'],
    }
...
if __name__ == "__main__":
    ...
    print(f"\nBatch shapes:")
    print(f"  historical:  {batch['historical'].shape}")    # (64, 48, 8)
