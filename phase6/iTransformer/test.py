"""
Test script for Phase 6 iTransformer — Correlation-Only Forecasting.
...
def denormalize(data, norm_params):
    ...
    Args:
        ...
        norm_params: dict with 'corr_mean'/'means' and 'corr_std'/'stds'
...
def run_inference(model, test_loader, device):
    ...
    for batch in test_loader:
        ...
        all_historical.append(batch['historical'].numpy())  # (B, 48, 8)

    predictions = np.concatenate(all_predictions, axis=0)  # (N, 12, 7)
    targets = np.concatenate(all_targets, axis=0)          # (N, 12, 7)
    historical = np.concatenate(all_historical, axis=0)    # (N, 48, 8)
...
def plot_sample_forecasts(predictions, targets, historical_corr, output_dir, n_samples=4):
    ...
    Each feature gets its own figure with n_samples examples showing full input + forecast.

    Args:
        predictions: (N, 12, 7) denormalized predictions in correlation space
        targets: (N, 12, 7) denormalized targets in correlation space
        historical_corr: (N, 48, 7) denormalized historical input (correlation channels only)
        output_dir: where to save
        n_samples: number of examples per feature plot
    """
    ...
    # Time axis: history is negative, forecast is positive (in minutes)
    t_hist = np.arange(-n_hist, 0) * 2.5      # [-120, -117.5, ..., -2.5]
    t_pred = np.arange(0, n_pred) * 2.5       # [0, 2.5, ..., 27.5]
    ...
    for c_idx, c_name in enumerate(CORRELATION_NAMES):
        ...
        fig.suptitle(f'iTransformer — {c_name}', fontsize=13, y=1.01)
        plt.tight_layout()
...
def plot_error_by_horizon(predictions, targets, output_dir):
    ...
    time_steps = np.arange(n_steps) * 2.5  # minutes
    ...
    ax.set_title('iTransformer — MAE by Forecast Horizon')
...
def plot_scatter(predictions, targets, output_dir):
    ...
    plt.suptitle('iTransformer — Predicted vs Actual', fontsize=14)
    plt.tight_layout()
...
def plot_metrics_bar(metrics, output_dir):
    ...
    plt.suptitle('iTransformer — Test Metrics', fontsize=14)
    plt.tight_layout()
...
def plot_bland_altman(predictions, targets, output_dir):
    ...
    X-axis: mean of predicted and actual
    Y-axis: difference (predicted - actual)
    Shows mean bias and ±1.96 SD limits of agreement.
    """
    fig, axes = plt.subplots(2, 4, figsize=(16, 8))
    axes = axes.flatten()
    ...
    plt.suptitle('iTransformer — Bland-Altman Plots', fontsize=14)
    plt.tight_layout()
...
def test(args):
    ...
    # Denormalize historical (first 7 channels are correlations, channel 8 is time)
    ...
    # Print results
    print(f"\n{'='*70}")
    print(f"  iTransformer Test Results (Phase 6 — Correlation Forecasting)")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test iTransformer for correlation forecasting")
