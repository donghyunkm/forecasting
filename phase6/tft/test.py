#!/usr/bin/env python3
"""
Phase 6 TFT Testing: Evaluate trained model on test set.
...
def denormalize(data, norm_params):
    ...
    Args:
        ...
        norm_params: dict with 'means' and 'stds' lists (length 7)
...
def plot_sample_forecasts(predictions, targets, historical_corr, output_dir, n_samples=4):
    ...
    for c_idx, c_name in enumerate(CORRELATION_NAMES):
        ...
        fig.suptitle(f'TFT — {c_name}', fontsize=13, y=1.01)
        plt.tight_layout()
...
def plot_error_by_horizon(predictions, targets, output_dir):
    ...
    ax.set_title('TFT — MAE by Forecast Horizon')
...
def plot_scatter(predictions, targets, output_dir):
    ...
    plt.suptitle('TFT — Predicted vs Actual', fontsize=14)
    plt.tight_layout()
...
def plot_metrics_bar(metrics, output_dir):
    ...
    plt.suptitle('TFT — Test Metrics', fontsize=14)
    plt.tight_layout()
...
def plot_bland_altman(predictions, targets, output_dir):
    ...
    plt.suptitle('TFT — Bland-Altman Plots', fontsize=14)
    plt.tight_layout()
...
def main():
    parser = argparse.ArgumentParser(description='Phase 6 TFT Testing')
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            all_historical.append(batch['historical_ts_numeric'].numpy())  # (batch, 48, 8)
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("TEST RESULTS — Phase 6 TFT (Correlation Forecasting)")
    print("=" * 70)
