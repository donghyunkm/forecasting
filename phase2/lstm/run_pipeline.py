#!/usr/bin/env python3
"""
run_pipeline.py - Orchestrator for heart rate prediction pipeline (LSTM).

Runs the full pipeline:
    1. Download data (if not already present)
    2. Train LSTM model for HR prediction
    3. Test model with saved checkpoint
    4. Plot predictions vs ground truth

Usage:
    python run_pipeline.py
    python run_pipeline.py --epochs 50 --input-length 7500 --target-length 7500
"""
...
def run_training(num_epochs, input_length, target_length):
    """Train LSTM model for HR prediction."""
    print("\n" + "=" * 60)
    print(f"STEP 2: Model Training ({num_epochs} epochs, in={input_length}, tgt={target_length})")
    print("=" * 60)
...
def run_testing(num_epochs, input_length, target_length):
    ...
    print("\n" + "=" * 60)
    print("STEP 3: Testing (loading best checkpoint)")
    print("=" * 60)
...
def main():
    """Run the full heart rate prediction pipeline."""
    from preprocess import INPUT_LENGTH, TARGET_LENGTH
    from model import NUM_EPOCHS

    parser = argparse.ArgumentParser(description='Run full HR prediction pipeline (LSTM)')
    ...
    print("=" * 60)
    print("Heart Rate Prediction Pipeline (LSTM)")
    print("=" * 60)
