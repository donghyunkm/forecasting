#!/usr/bin/env python3
"""
test.py - Standalone testing script for all 3 diffusion forecasting models.

Loads the best checkpoint for each signal (ABP, PLETH, II), runs the full
reverse diffusion process to generate predictions, and saves results.
...
def test_single_model(target_idx, device):
    ...
    model.eval()
    print(f"[{signal_name}] Model loaded ({sum(p.numel() for p in model.parameters()):,} params)")
    ...
    print(f"[{signal_name}] Running reverse diffusion ({num_timesteps} steps per sample)...")
    all_predictions = []
    all_targets = []
    batch_count = 0
    ...
    with torch.no_grad():
        for x_cond, y_target in test_loader:
            ...
            # Generate forecast via full reverse diffusion
            predictions = sample(model, x_cond, diffusion_params, device)
    ...
    print(f"[{signal_name}] Done — MAE(raw): {mae_raw:.4f}, RMSE(raw): {rmse_raw:.4f}")
...
def run_test():
    ...
    # Print summary
    print("\n" + "=" * 60)
    print("TEST RESULTS — Diffusion Models (All Signals)")
    print("=" * 60)
    ...
    print(f"Diffusion steps: {all_metrics[SIGNAL_NAMES[0]]['num_diffusion_steps']}")
    print("=" * 60)
