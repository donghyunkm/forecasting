#!/usr/bin/env python3
"""
test.py - Evaluation for TFT-multi vital sign forecasting (Phase 4).

Adapted from the TFT-multi notebook's evaluation cells:
  - Per-vital MAE/MAPE for each quantile
  - Percent bounded (calibration): % of true values within 10th-90th bounds
  - Per-patient calibration statistics
...
def test_model(device, num_epochs=NUM_EPOCHS):
    ...
    Evaluation logic adapted from notebook's metric calculation cells.
    """
    ...
    norm_mean = np.array(norm_params['mean'])   # (5,)
    norm_std = np.array(norm_params['std'])     # (5,)
    ...
    # Run inference (matching notebook's process_test_batch)
    ...
    predictions = np.concatenate(all_predictions, axis=0)  # (N, 25, 15)
    targets = np.concatenate(all_targets, axis=0)          # (N, 25, 5) normalized
    masks = np.concatenate(all_masks, axis=0)              # (N, 25, 5)

    # Reshape predictions: (N, 25, 15) → (N, 25, 5, 3)
    predictions = predictions.reshape(predictions.shape[0], FUTURE_MONTHS, NUM_SIGNALS, len(OUTPUT_QUANTILES))

    # Denormalize predictions and targets
    predictions_denorm = predictions * norm_std[None, None, :, None] + norm_mean[None, None, :, None]
    ...
    for meas in range(NUM_SIGNALS):
        ...
        pred_meas = predictions_denorm[:, :, meas, :]  # (N, 25, 3)
        true_meas = targets_denorm[:, :, meas]          # (N, 25)
        mask_meas = masks[:, :, meas]                   # (N, 25)
            ...
            for j in range(FUTURE_MONTHS):
                if mask_meas[i, j] == 1 and true_meas[i, j] > 0:
                    ...
                    for q_idx in range(num_quantiles):
                        ...
                        mape_per_q[q_idx] += np.abs((pred_meas[i, j, q_idx] - y) / y)
    ...
    pred_median = predictions_denorm[:, :, :, median_idx]  # (N, 25, 5)

    # Masked overall MAE/RMSE
    valid_mask = masks.astype(bool) & (targets_denorm > 0)
    overall_mae = float(np.mean(np.abs(pred_median[valid_mask] - targets_denorm[valid_mask])))
    overall_rmse = float(np.sqrt(np.mean((pred_median[valid_mask] - targets_denorm[valid_mask]) ** 2)))
    ...
    # Overall calibration
    overall_calibration = float(np.mean([per_vital_calibration[n] for n in SIGNAL_NAMES]))
    ...
    metrics = {
        ...
        'forecast_hours': FUTURE_MONTHS,
        'num_signals': NUM_SIGNALS,
        'signal_names': SIGNAL_NAMES,
        'quantiles': OUTPUT_QUANTILES,
        'input_hours': PAST_MONTHS,
...
def run_test(num_epochs=NUM_EPOCHS):
    ...
    # Print summary (matching notebook output style)
    print("\n" + "=" * 60)
    print("TEST RESULTS — TFT-multi Vital Sign Forecasting")
    print("=" * 60)
    print(f"  Input:  {PAST_MONTHS} hours")
    print(f"  Output: {FUTURE_MONTHS} hours × {NUM_SIGNALS} vitals × "
          f"{len(OUTPUT_QUANTILES)} quantiles")
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Evaluate TFT-multi forecaster')
