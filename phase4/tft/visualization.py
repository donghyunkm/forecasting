"""
visualization.py - Visualization functions for TFT-multi output.

Exact copy from rosie068/TFT-multi (adapted from PlaytikaOSS/tft-torch).

Functions:
  - display_target_trajectory: Plot forecast with confidence bands
  - display_attention_scores: Attention mechanism visualization
  - display_selection_weights_stats: Variable selection weights
  - display_sample_wise_attention_scores: Per-sample attention
  - display_sample_wise_selection_stats: Per-sample selection weights
"""
...
def aggregate_weights(output_arr: np.ndarray, prctiles: List[float],
...
def display_selection_weights_stats(outputs_dict: Dict[str, np.ndarray],
    ...
    for name, config in mapping.items():
        ...
        print(name)
        print('=========')
        print(weights_agg.sort_values([sort_by], ascending=False))
...
def display_attention_scores(attention_scores: np.ndarray,
    ...
    if len(prctiles) == 1:
        ...
        for horizon in horizons:
            ...
            ax.plot(x_axis, scores_percentile[horizon - 1], lw=1,
                    label=f"t + {horizon} scores", marker='o')
    else:
        title = f"{horizons[0]} Steps Ahead - Attention Scores"
        for prctile, scores_percentile in attn_stats.items():
            siz = scores_percentile.shape
            x_axis = np.arange(siz[0] - siz[1], siz[0])
...
def display_target_trajectory(signal_history: np.ndarray,
    ...
    """
    Plot a single forecast example with confidence bands.

    Exact from TFT-multi repo, with optional save_path for non-interactive use.
    """
    ...
    ax.plot(past_x, transformation(past[np.newaxis, ...]).reshape(-1),
            lw=3, label='observed', marker='o')
    ax.plot(fut_x, transformation(future[np.newaxis, ...]).reshape(-1),
            lw=3, label='target', marker='o')

    # for each predicted quantile, plot the quantile prediction
    for idx, quantile in enumerate(model_quantiles):
        ax.plot(fut_x, transformation(preds[np.newaxis, ..., idx]).reshape(-1),
                linestyle='--', lw=2, marker='s', label=f"predQ={quantile}")
...
def display_sample_wise_selection_stats(weights_arr: np.ndarray,
    ...
    if is_temporal:
        ...
        weights_df = pd.DataFrame({'weight': sample_weights_trans.mean(axis=1)},
                                  index=feature_names)
    else:
