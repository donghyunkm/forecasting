#!/usr/bin/env python3
"""
test.py - Evaluation for iTransformer vital sign forecasting (Phase 42).
...
def test_model(device, num_epochs=NUM_EPOCHS):
    ...
    norm_json = os.path.join('/gpfs/scratch/dk5565/phase42_data/processed', 'norm_params.json')
    with open(norm_json, 'r') as f:
        norm_data = json.load(f)
    norm_mean = np.array(norm_data['mean'])
    norm_std = np.array(norm_data['std'])

    # Recreate model with use_mask_input=False
    model = iTransformer(
...
def run_test(num_epochs=NUM_EPOCHS):
    ...
    print("\n" + "=" * 60)
    print("TEST RESULTS — iTransformer Vital Sign Forecasting (Phase 42)")
    print("=" * 60)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Evaluate iTransformer forecaster (Phase 42 — 4 vitals, 15-min)')
