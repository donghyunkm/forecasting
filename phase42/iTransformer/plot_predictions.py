#!/usr/bin/env python3
"""
plot_predictions.py - Generate forecast visualizations for iTransformer Phase 42.
...
def plot_forecast_samples(predictions, targets, masks, output_dir, num_samples=5):
    ...
    for sample_i in range(min(num_samples, len(predictions))):
        ...
        plt.suptitle(
            f'iTransformer Phase 42 — Forecast Sample {sample_i + 1}\n'
...
def plot_scatter_per_vital(predictions, targets, masks, output_dir):
    ...
    plt.suptitle('iTransformer Phase 42 — Predicted vs Actual (Median)', fontsize=14)
    plt.tight_layout()
...
def plot_error_by_step(predictions, targets, masks, output_dir):
    ...
    ax.set_ylabel('MAE')
    ax.set_title('iTransformer Phase 42 — MAE by Forecast Step (15-min resolution)')
    ax.legend()
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_calibration_summary(predictions, targets, masks, output_dir):
    ...
    ax.set_title('iTransformer Phase 42 — Prediction Interval Calibration')
    ax.set_ylim(0, 1.0)
    ax.legend()
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Plot iTransformer results (Phase 42 — 4 vitals, 15-min)')
