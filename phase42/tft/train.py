#!/usr/bin/env python3
"""
train.py - Training script for TFT vital sign forecasting (Phase 4.2).
...
def train_model(device, num_epochs=NUM_EPOCHS):
    """
    Train TFT model — adapted from the notebook's training loop.
    """
    ...
    data_props = {
        'num_historical_numeric': 5,  # 4 vitals + 1 time (no masks in Phase 4.2)
        ...
        'num_feature_predicted': NUM_SIGNALS,  # 4
    }
    ...
    ax.set_title('TFT — Vital Sign Forecasting, Complete Windows (Phase 4.2) — Training Curves')
    ax.legend()
...
def main(num_epochs=None):
    ...
    print("=" * 60)
    print("TFT — Vital Sign Forecasting, Complete Windows (Phase 4.2)")
    print("=" * 60)
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Train TFT for vital sign forecasting (Phase 4.2)')
