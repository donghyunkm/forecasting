"""
Training script for Phase 5 iTransformer ablation (vitals-only).

Same training logic as the full Phase 5 iTransformer but with vitals-only input (5 features).
...
def train_epoch(model, loader, optimizer, device, grad_clip=1.0):
    ...
    for batch in loader:
        historical = batch['historical'].to(device)   # (B, 72, 5)
...
def train(args):
    ...
    print(f"    Ablation: vitals-only (no correlation features)")
...
def save_training_curves(train_losses, val_losses, best_epoch, output_dir):
    ...
    ax.set_title('iTransformer Training Curves (Phase 5 — Vitals-Only Ablation)')
    ax.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train iTransformer ablation (vitals-only)")
