"""
Test script for Phase 5 iTransformer vital sign forecasting.

Loads best model, runs inference on test set, denormalizes predictions,
and computes per-vital MAE/MAPE/calibration/correlation and overall MAE/RMSE.
"""
...
def denormalize_vitals(predictions_norm, targets_norm, norm_params):
    ...
    Vital signs are at indices 7-10 in the 11-dim normalization.
...
def compute_metrics(predictions, targets, masks):
    ...
    for v_idx, v_name in enumerate(VITAL_NAMES):
        ...
        valid = mask_v > 0.5
        if valid.sum() == 0:
            metrics[v_name] = {'MAE': float('nan'), 'MAPE': float('nan'),
                               'calibration_80': float('nan'), 'correlation': float('nan')}
            continue
...
def test(args):
    ...
    # Inference
    print("\n--- Running Inference ---")
    predictions_norm, targets, masks = run_inference(model, test_loader, device)
    ...
    print("\n--- Denormalizing ---")
    predictions, targets = denormalize_vitals(predictions_norm, targets, norm_params)
    ...
    # Print results
    print(f"\n{'='*70}")
    print(f"  iTransformer Test Results (Phase 5)")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test iTransformer for vital sign forecasting")
