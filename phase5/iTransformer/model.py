...
class iTransformer(nn.Module):
    """
    iTransformer for quantile vital sign forecasting.

    Input: (B, seq_len=72, n_input_vars=12)
        - indices 0-6: correlation features
        - indices 7-10: vital signs (ABPMean, PULSE, SpO2, RESP)
        - index 11: time feature
    ...
    def __init__(
        ...
        seq_len: int = 72,
        pred_len: int = 24,
        n_vars: int = 11,          # 7 correlations + 4 vitals (excluding time)
        n_input_vars: int = 12,    # 7 correlations + 4 vitals + 1 time
        ...
        # Indices of the vital sign variates in the input (7, 8, 9, 10)
        self.vital_indices = list(range(7, 7 + n_output_vars))
    ...
    def forward(self, x):
        """
        Args:
            x: (B, seq_len, n_input_vars) = (B, 72, 12)
        ...
        # Embed: (B, 72, 12) -> (B, 12, d_model)
        enc_in = self.embedding(x)

        # Encode with transformer (attention across 12 variate tokens)
        enc_out = self.encoder(enc_in)  # (B, 12, d_model)

        # Extract vital sign tokens (indices 7, 8, 9, 10)
...
def build_model(device='cuda'):
    """Build iTransformer with default config."""
    model = iTransformer(
        seq_len=72,
        pred_len=24,
        n_vars=11,
...
if __name__ == "__main__":
    ...
    # Test forward pass
    x = torch.randn(4, 72, 12).to(device)
    out = model(x)
